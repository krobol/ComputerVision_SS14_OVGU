...
int main(int argc, char** argv)
{
	...
	while (true)
	{
		...
		if(pause == false){
			...
			medianBlur(mask, mask, 7); // TODO schnelleren filter finden!
			threshold(mask, mask, 128, 255, THRESH_BINARY);
			erode(mask, mask, Mat::ones(5, 5, CV_8UC1));
			...
			if (!prev.empty())
			{
				smoothed = prev & denoised;
			}
			else
			{
				smoothed = Mat::ones(frame.size[0], frame.size[1], CV_8UC1);
			}

			denoised.copyTo(prev);
			erode(smoothed, smoothed, Mat::ones(5, 5, CV_8UC1));
			dilate(smoothed, smoothed, Mat::ones(40, 50, CV_8UC1));

			result = denoised & smoothed;
